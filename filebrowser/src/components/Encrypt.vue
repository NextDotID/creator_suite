  ...
  <v-card class="mx-auto" max-width="500">
    ...
    <v-window v-model="step">
      <v-window-item :value="1">
        <v-card-text>
          <v-text-field prepend-icon="mdi-paperclip" label="Encrypt File" placeholder="Your File"
            v-model="origin_file"></v-text-field>
          <span class="text-caption text-grey-darken-1">

          </span>
          <v-text-field prepend-icon="mdi-shield-key-outline" label="Password" type="password" v-model="key"
            hint="Please enter a password"></v-text-field>
      ...
      <v-window-item :value="2">
        <v-card-text>
          ...
          <v-text-field label="API Port" v-model="store.api_port"></v-text-field>
          <v-text-field label="GateWay Port" v-model="store.gateway_port"></v-text-field>
          <v-text-field label="PeerID" type="password" v-model="store.peer_id"></v-text-field>
          <v-text-field label="Public Key" type="password" v-model="store.pub_key"></v-text-field>
          <span class="text-caption text-grey-darken-1">
...
export default defineComponent({
  ...
  methods: {
    ...
    async test_img() {
      var a: File = this.selectedFiles[0]
      var arr = await a.arrayBuffer()
      var view = new DataView(arr)
      var c = new Blob([view.buffer]);
    },
    create() {
      var req: CreateRequest = {
        encrypt_type: Number(this.type),
        key: this.key,
    ...
    create_content() {
      var req: CreateContentRequest = {
        ...
        encryption_type: Number(this.type),
